...
async function fetchTransactions(setOutput) {
    ...
    for (let i = 0; i < celebrityList.length; i++) {
        ...
        let URL1 = `https://api.etherscan.io/api?module=account&action=txlist&address=${celebrityList[i][1]
            }&startblock=${blockIndex - 40000
            ...
            .then((response) => {
                ...
                for (let j = 0; j < results.length; j++) {
                    ...
                    if (Number(results[j].value) >= 4 * 10 ** 18) {
                        setOutput((prev) => [
                            ...prev,
                            createData(results[j].value / 10 ** 18, "ETH", results[j].from, results[j].to,
                                results[j].from.toLowerCase() == celebrityList[i][1].toLowerCase() ?
                                    "Tx is Out From" :
                                    "Tx is Send To"
                                , celebrityList[i][0])
                            // "Address " +
        ...
        let URL2 = `https://api.etherscan.io/api?module=account&action=txlistinternal&address=${celebrityList[i][1]
            }&startblock=${blockIndex - 40000
            }&endblock=${blockIndex}&sort=desc&apikey=${YourApiKeyToken}`;
        ...
        await axios
            ...
            .then((response) => {
                ...
                for (let j = 0; j < results.length; j++) {
                    //If the value is more than 10 ETH in one transaction, I print the info.
                    if (Number(results[j].value) >= 4 * 10 ** 18) {
                        console.log(results[j].from)
                        console.log(celebrityList[i][1])
                        setOutput((prev) => [
                            ...prev,
                            createData(results[j].value / 10 ** 18, "ETH", results[j].from, results[j].to, 
                                results[j].from.toLowerCase() == celebrityList[i][1].toLowerCase() ?
...
function createData(eth, token, from, to, a, b) {
    return { eth, token, from, to, a, b };
}
                    ...
                    <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell align="right">Celebrity&nbsp;</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {rows.map((row) => (
